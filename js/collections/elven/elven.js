...
(() => {
  ...
  // Bind to this page's side-column images (and future card images if you add them)
  const imgs = document.querySelectorAll('.elven-side-column img, .card-image img');
  imgs.forEach(img => {
    img.style.cursor = 'zoom-in';
    img.addEventListener('click', () => {
      const fullSrc = img.getAttribute('data-full') || img.currentSrc || img.src;
      const alt = img.getAttribute('alt') || '';
      // Use page banner as caption, fallback to alt
