...
(() => {
  ...
  // Bind to this page's side-column images (and card images if present)
  const imgs = document.querySelectorAll('.siren-side-column img, .card-image img');
  imgs.forEach(img => {
    img.style.cursor = 'zoom-in';
    img.addEventListener('click', () => {
      const fullSrc = img.getAttribute('data-full') || img.currentSrc || img.src;
      const alt = img.getAttribute('alt') || '';
      // Use the banner heading as caption; fall back to alt
