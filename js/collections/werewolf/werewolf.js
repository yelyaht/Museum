...
(() => {
  ...
  // Bind to this page's side-column images (and support future card images)
  const imgs = document.querySelectorAll('.werewolf-side-column img, .card-image img');
  imgs.forEach(img => {
    img.style.cursor = 'zoom-in';
    img.addEventListener('click', () => {
      const fullSrc = img.getAttribute('data-full') || img.currentSrc || img.src;
      const alt = img.getAttribute('alt') || '';
      // Use banner title as caption; fallback to alt
